...
class MamlDataset(Dataset):
    def __init__(self, root, mode, batchsz, n_way, k_shot, k_query,imgsz,imgc):
    ...
    def render_img(self, arr):
        ...
        if self.imgc == 3:
            ...
            img = Image.fromarray(np.squeeze(arr), mode='L')
        return img
    ...
    def __getitem__(self, index):
        ...
        support_x = torch.FloatTensor(self.setsz, self.imgc, self.imgsz, self.imgsz)
        # [querysz, c, resize, resize]
        query_x = torch.FloatTensor(self.querysz, self.imgc, self.imgsz, self.imgsz)
